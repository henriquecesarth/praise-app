...
import {
  ...
  isEarlyActivationCheckoutAttemptExpired,
...
export class BillingService {
  ...
  /**
   * Consulta os dados da assinatura de forma estrita (Phase 3D.2 Hardening).
   * Exige getSubscriptionState no provedor. Nunca faz fallback silencioso para semântica legada.
   */
  private async getProviderSubscriptionStateStrict(
    providerSubscriptionId: string
  ): Promise<ProviderSubscriptionState> {
    if (typeof (this.provider as any).getSubscriptionState === 'function') {
      return await (this.provider as any).getSubscriptionState(providerSubscriptionId);
    }

    return {
      outcome: 'TRANSIENT_ERROR',
      errorMessage: 'STRICT_PROVIDER_CONTRACT_UNAVAILABLE: Provedor não implementa getSubscriptionState.',
    };
  }

  /**
   * Executa inativação no gateway de forma estrita para V1 (Phase 3D.2 Hardening).
   * Exige inactivateSubscriptionStrict no provedor. Nunca faz fallback silencioso para semântica legada.
   */
  private async inactivateProviderSubscriptionStrict(
    providerSubscriptionId: string
  ): Promise<ProviderSubscriptionInactivateResult> {
    if (typeof (this.provider as any).inactivateSubscriptionStrict === 'function') {
      return await (this.provider as any).inactivateSubscriptionStrict(providerSubscriptionId);
    }

    return {
      outcome: 'TRANSIENT_ERROR',
      errorMessage: 'STRICT_PROVIDER_CONTRACT_UNAVAILABLE: Provedor não implementa inactivateSubscriptionStrict.',
    };
  }

  /**
   * Lista de forma exaustiva, paginada e fail-closed todas as cobranças da exata assinatura de origem (Phase 3D.2 Hardening).
   * Consome todas as páginas do provedor, deduplica pagamentos e detecta conflitos de estado.
   */
  private async listExactSourcePaymentsExhaustive(
    sourceSubId: string
  ): Promise<
    | { outcome: 'SUCCESS'; payments: ProviderPaymentRecord[] }
    | { outcome: 'ATTENTION'; reason: string }
    | { outcome: 'TRANSIENT'; reason: string }
  > {
    if (typeof (this.provider as any).listAllSubscriptionPaymentsStrict === 'function') {
      try {
        const strictRes = await (this.provider as any).listAllSubscriptionPaymentsStrict(sourceSubId);
        if (strictRes.outcome === 'NOT_FOUND') {
          return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_RESOURCE_DIVERGENCE };
        }
        if (strictRes.outcome === 'AUTH_ERROR') {
          return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_AUTH_FAILURE };
        }
        if (strictRes.outcome === 'MALFORMED_RESPONSE') {
          return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.MALFORMED_PROVIDER_PAYMENT };
        }
        if (strictRes.outcome === 'TRANSIENT_ERROR') {
          return { outcome: 'TRANSIENT', reason: strictRes.errorMessage || 'transient_provider_payment_list_error' };
        }
        const rawPayments = strictRes.payments || [];
        ...
        for (const p of rawPayments) {
          ...
          if (seenMap.has(p.id)) {
            ...
            if (prev.status !== p.status) {
              console.error(`[PAYMENT SAFETY GATE] Conflito de estado na cobrança ${p.id}: ${prev.status} vs ${p.status}`);
              return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.MALFORMED_PROVIDER_PAYMENT };
            }
          } else {
            seenMap.set(p.id, p);
        ...
        return { outcome: 'SUCCESS', payments: Array.from(seenMap.values()) };
      } catch (err: any) {
        return { outcome: 'TRANSIENT', reason: err.message || 'transient_provider_payment_list_error' };
      }
    }

    if (typeof this.provider.listSubscriptionPayments === 'function') {
      try {
        const pList = await this.provider.listSubscriptionPayments(sourceSubId, { status: 'ALL' });
        ...
        if (err instanceof AppError) {
          if (err.statusCode === 404 || (err as any).status === 404) {
            return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_RESOURCE_DIVERGENCE };
          }
          if (err.statusCode === 401 || err.statusCode === 403) {
            return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_AUTH_FAILURE };
          }
          if (err.message && (err.message.includes('malformado') || err.message.includes('Conflito') || err.message.includes('Anomalia') || err.message.includes('Limite de paginação'))) {
            return { outcome: 'ATTENTION', reason: CANCEL_TO_FREE_ATTENTION_REASONS.MALFORMED_PROVIDER_PAYMENT };
          }
        }
        return { outcome: 'TRANSIENT', reason: err.message || 'transient_payment_list_error' };
      }
    }
  ...
  /**
   * Phase 3D.2: Reconciliação do-not-renew para scheduled_cancel_to_free.
   *
   * Responsabilidades:
   * 1. Trava técnica distribuída e validação V1 (scheduled_cancel_to_free).
   * 2. Gate de Propriedade do Active Transition Slot (Section 9).
   * 3. Exact Source Subscription Scope: usa exclusivamente old_provider_subscription_id (Section 7).
   * 4. Strict Inactivation Gate: GET/PUT status INACTIVE comprovado via fresh provider proof (Sections 10 & 11).
   * 5. Payment Safety Gate: filtra dueDate >= effective_billing_date da exata assinatura de origem (Section 12).
   * 6. Limpeza cirúrgica de cobranças PENDING futuras (Section 13) e preservação de histórico anterior (Section 15).
   * 7. Fail-closed imediato em caso de conflitos financeiros (CONFIRMED, RECEIVED, OVERDUE, malformed) (Section 2).
   * 8. Re-leitura obrigatória comprovando convergência antes de promover (Section 18).
   * 9. Promoção para scheduled SOMENTE quando DO-NOT-RENEW SAFETY estiver provada. Slot permanece HELD (Section 19).
   */
  async reconcileScheduledCancelToFreeDoNotRenew(
    transitionId: string,
    actor: string = 'worker',
    options?: { now?: Date }
    ...
    try {
      ...
      // Se já alcançou scheduled, idempotente NO-OP
      if (claimed.transition_status === 'scheduled') {
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: true, reason: 'already_scheduled', transition: claimed };
      }

      // Se em atenção financeira, não muta
      if (
      ...
      // Precondição: deve estar em awaiting_old_inactivation
      ...
      // Gate de Propriedade do Active Transition Slot (Section 9)
      ...
      if (
        ...
        console.error(`[DO-NOT-RENEW GUARD] Slot ativo ausente ou divergente para transição ${claimed.id}`);
      ...
      // Exact Source Subscription (Section 7)
      const sourceSubId = claimed.old_provider_subscription_id || claimed.previous_provider_subscription_id;
      if (!sourceSubId) {
        console.error(`[DO-NOT-RENEW GUARD] Missing source subscription ID for transition ${claimed.id}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      if (!cutoffDate) {
        console.error(`[DO-NOT-RENEW GUARD] Missing effective_billing_date for transition ${claimed.id}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      // Gate de Capability Estrita do Provedor (Phase 3D.2 Hardening - Section 2)
      if (
        typeof (this.provider as any).getSubscriptionState !== 'function' ||
        typeof (this.provider as any).inactivateSubscriptionStrict !== 'function'
      ) {
        console.error(`[DO-NOT-RENEW GUARD] Provedor ${this.provider.name} não suporta contrato estrito V1 (getSubscriptionState / inactivateSubscriptionStrict).`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
      ...
      // STEP 1: Strict Inactivation Gate (Sections 10 & 11)
      const subState = await this.getProviderSubscriptionStateStrict(sourceSubId);

      if (subState.outcome === 'NOT_FOUND') {
        console.error(`[DO-NOT-RENEW GUARD] Source subscription ${sourceSubId} 404 (resource divergence)`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
          financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_RESOURCE_DIVERGENCE,
          financial_safety_status: 'attention_required',
      ...
      if (subState.outcome === 'AUTH_ERROR') {
        console.error(`[DO-NOT-RENEW GUARD] Provider auth failure on GET subscription ${sourceSubId}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      if (subState.outcome === 'MALFORMED_RESPONSE') {
        console.error(`[DO-NOT-RENEW GUARD] Malformed provider subscription response for ${sourceSubId}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      if (subState.status === 'INACTIVE') {
        if (!claimed.source_inactivation_confirmed_at) {
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            source_inactivation_confirmed_at: nowIso,
            updated_at: nowIso,
          });
          claimed.source_inactivation_confirmed_at = nowIso;
        }
      } else if (subState.status === 'ACTIVE') {
        if (!claimed.source_inactivation_attempted_at) {
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            source_inactivation_attempted_at: nowIso,
            updated_at: nowIso,
          });
          claimed.source_inactivation_attempted_at = nowIso;
        }

        const inactResult = await this.inactivateProviderSubscriptionStrict(sourceSubId);
        if (inactResult.outcome === 'NOT_FOUND') {
          console.error(`[DO-NOT-RENEW GUARD] PUT status INACTIVE returned 404 for ${sourceSubId}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_RESOURCE_DIVERGENCE,
            financial_safety_status: 'attention_required',
        ...
        if (inactResult.outcome === 'AUTH_ERROR') {
          console.error(`[DO-NOT-RENEW GUARD] PUT status INACTIVE returned auth error for ${sourceSubId}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_AUTH_FAILURE,
            financial_safety_status: 'attention_required',
        ...
        if (inactResult.outcome === 'CLIENT_ERROR') {
          console.error(`[DO-NOT-RENEW GUARD] PUT status INACTIVE returned client error 400 for ${sourceSubId}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_CLIENT_ERROR,
            financial_safety_status: 'attention_required',
            updated_at: new Date().toISOString(),
          });
          await this.billingRepo.releasePlanChangeLock(claimed.id);
        ...
        if (inactResult.outcome === 'TRANSIENT_ERROR') {
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            last_reconciled_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          });
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return { success: false, reason: 'uncertain_put_timeout' };
        }

        // Fresh GET proof após PUT 200
        const freshProof = await this.getProviderSubscriptionStateStrict(sourceSubId);
        if (freshProof.outcome === 'TRANSIENT_ERROR') {
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            last_reconciled_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          });
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return { success: false, reason: 'transient_fresh_read_error' };
        }

        if (freshProof.outcome !== 'FOUND' || freshProof.status !== 'INACTIVE') {
          console.error(`[DO-NOT-RENEW GUARD] Fresh read after PUT did not prove INACTIVE: status=${freshProof.status}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: freshProof.outcome === 'NOT_FOUND'
          ...
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return { success: false, reason: 'source_inactivation_not_proven' };
        }

        const confirmedIso = new Date().toISOString();
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          source_inactivation_confirmed_at: confirmedIso,
          updated_at: confirmedIso,
        });
        claimed.source_inactivation_confirmed_at = confirmedIso;
      } else {
        console.error(`[DO-NOT-RENEW GUARD] Source subscription status unexpected: ${subState.status}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      // STEP 2: Payment Safety Gate (Sections 12, 13, 14, 15, 16, 17, 18, Hardening Gates 1 & 2)
      const listRes = await this.listExactSourcePaymentsExhaustive(sourceSubId);
      if (listRes.outcome === 'ATTENTION') {
        console.error(`[PAYMENT SAFETY GATE] Falha estrita ao listar cobranças da source ${sourceSubId}: ${listRes.reason}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      // Filtrar obrigações para a assinatura exata de origem com dueDate >= cutoffDate
      ...
      for (const pay of futureObligations) {
        if (!pay.status || typeof pay.status !== 'string') {
          console.error(`[PAYMENT SAFETY GATE] Cobrança malformada encontrada: ${pay.id}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
        ...
        if (pay.status === 'CONFIRMED' || pay.status === 'RECEIVED' || pay.status === 'RECEIVED_IN_CASH') {
          console.error(`[PAYMENT SAFETY GATE] Cobrança liquidada encontrada na fronteira >= ${cutoffDate}: ${pay.id} (${pay.status})`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.FUTURE_SETTLED_OBLIGATION_DETECTED,
            financial_safety_status: 'attention_required',
            updated_at: new Date().toISOString(),
          });
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return { success: false, reason: 'future_settled_obligation_detected' };
        }

        if (pay.status === 'OVERDUE') {
          console.error(`[PAYMENT SAFETY GATE] Cobrança vencida (OVERDUE) encontrada na fronteira >= ${cutoffDate}: ${pay.id}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
        ...
        if (pay.status !== 'PENDING') {
          console.error(`[PAYMENT SAFETY GATE] Cobrança com status não permitido na fronteira >= ${cutoffDate}: ${pay.id} (${pay.status})`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.MALFORMED_PROVIDER_PAYMENT,
            financial_safety_status: 'attention_required',
      ...
      for (const pay of pendingToDelete) {
        try {
          ...
          console.warn(`[PAYMENT CLEANUP] Erro ao remover cobrança ${pay.id}: ${delErr.message}`);
          ...
          if (
            ...
            console.error(`[PAYMENT RACE] Cobrança ${pay.id} foi liquidada durante exclusão!`);
            await this.billingRepo.updateTransition(claimed.id, ministryId, {
              financial_attention_required: true,
              financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.CLEANUP_RACE_SETTLED,
              financial_safety_status: 'attention_required',
              updated_at: new Date().toISOString(),
            });
            await this.billingRepo.releasePlanChangeLock(claimed.id);
            return { success: false, reason: 'cleanup_race_settled' };
          }
      ...
      // Re-leitura obrigatória e exaustiva de todas as cobranças da exata assinatura de origem
      const freshListRes = await this.listExactSourcePaymentsExhaustive(sourceSubId);
      if (freshListRes.outcome === 'ATTENTION') {
        console.error(`[PAYMENT SAFETY GATE] Falha estrita na re-leitura de cobranças da source ${sourceSubId}: ${freshListRes.reason}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
      ...
      for (const rem of remainingFuture) {
        if (rem.status === 'CONFIRMED' || rem.status === 'RECEIVED' || rem.status === 'RECEIVED_IN_CASH') {
          console.error(`[PAYMENT RACE] Cobrança liquidada detectada após cleanup: ${rem.id}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.CLEANUP_RACE_SETTLED,
            financial_safety_status: 'attention_required',
        ...
        if (rem.status === 'OVERDUE') {
          console.error(`[PAYMENT SAFETY GATE] Cobrança OVERDUE detectada após cleanup: ${rem.id}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.FUTURE_OVERDUE_OBLIGATION_DETECTED,
            financial_safety_status: 'attention_required',
        ...
        if (rem.status === 'PENDING') {
          console.warn(`[PAYMENT CLEANUP] Cobrança PENDING ainda persiste após cleanup: ${rem.id}`);
          await this.billingRepo.updateTransition(claimed.id, ministryId, {
            payment_cleanup_ids: cleanedIds,
            last_reconciled_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
        ...
        console.error(`[PAYMENT SAFETY GATE] Cobrança excepcional detectada após cleanup: ${rem.id} (${rem.status})`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
          financial_attention_reason: CANCEL_TO_FREE_ATTENTION_REASONS.MALFORMED_PROVIDER_PAYMENT,
          financial_safety_status: 'attention_required',
      ...
      // Verificação final do status da assinatura de origem
      const finalSubCheck = await this.getProviderSubscriptionStateStrict(sourceSubId);
      if (finalSubCheck.outcome !== 'FOUND' || finalSubCheck.status !== 'INACTIVE') {
        console.error(`[DO-NOT-RENEW GUARD] Source subscription não está INACTIVE na verificação final: status=${finalSubCheck.status}`);
        await this.billingRepo.updateTransition(claimed.id, ministryId, {
          financial_attention_required: true,
          financial_attention_reason: finalSubCheck.outcome === 'NOT_FOUND'
            ? CANCEL_TO_FREE_ATTENTION_REASONS.PROVIDER_RESOURCE_DIVERGENCE
            : CANCEL_TO_FREE_ATTENTION_REASONS.SOURCE_SUBSCRIPTION_UNEXPECTED_STATUS,
          financial_safety_status: 'attention_required',
          updated_at: new Date().toISOString(),
        ...
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: false, reason: 'source_not_inactive_at_boundary' };
      }

      // PROMOÇÃO PARA SCHEDULED (Sections 18 & 19)
      ...
      await this.billingRepo.updateTransition(claimed.id, ministryId, {
        ...
        payment_cleanup_ids: cleanedIds,
        payment_cleanup_completed_at: finalNowIso,
        source_inactivation_confirmed_at: claimed.source_inactivation_confirmed_at || finalNowIso,
      ...
      console.error(`[DO-NOT-RENEW ERROR] Falha inesperada ao reconciliar cancelamento ${claimed.id}:`, err);
      await this.billingRepo.releasePlanChangeLock(claimed.id);
  ...
  /**
   * Fluxo legacy de cancelamento preservado internamente para callers e testes legados que não instanciam
   * a infraestrutura de transições V1 / slots (Section 38).
   */
  async cancelSubscriptionLegacy(ministryId: string): Promise<BillingSubscriptionRecord> {
  ...
  /**
   * Agenda o cancelamento da assinatura ao fim do período atual pago (`cancel_at_period_end`) via fluxo V1 (Phase 3D.2).
   * 1. Prepara e persiste transição V1 com slot HELD.
   * 2. Tenta convergir imediatamente a inativação no provedor e limpeza cirúrgica de obrigações futuras.
   * 3. Retorna resposta segura e compatível.
   * NUNCA faz fallback silencioso para o fluxo legado (Section 1 e 5).
   */
  async cancelSubscriptionV1(
    ministryId: string,
    options?: { userId?: string; now?: Date | string }
  ): Promise<BillingSubscriptionRecord & { transition?: ScheduledCancelToFreeResponseDto }> {
    // 1. Preparar/reaproveitar transição V1
    const transition = await this.prepareScheduledCancelToFreeTransition(ministryId, options);

    // 2. Tentar convergir imediatamente do-not-renew
    let currentTransition = transition;
    if (currentTransition.transition_status === 'awaiting_old_inactivation') {
      const rec = await this.reconcileScheduledCancelToFreeDoNotRenew(
        currentTransition.id,
        options?.userId || 'public_cancel',
        options?.now ? { now: typeof options.now === 'string' ? new Date(options.now) : options.now } : undefined
      );
      if (rec.transition) {
        currentTransition = rec.transition;
      }
    }

    // 3. Compatibilidade retroativa: sincronizar flag de projeção cancel_at_period_end
    const nowIso = new Date().toISOString();
    const currentBillingSub = await this.billingRepo.getSubscription(ministryId, this.provider.name);
    let updatedBillingSub = currentBillingSub;
    if (currentBillingSub && !currentBillingSub.cancel_at_period_end) {
      updatedBillingSub = {
        ...currentBillingSub,
        cancel_at_period_end: true,
        updated_at: nowIso,
    ...
    // 4. Atenção financeira: se a transição falhar fechada com atenção, disparar erro com detalhes
    if (currentTransition.financial_attention_required) {
      throw new AppError(
        500,
        'Identificada pendência ou conflito financeiro no gateway. O cancelamento foi registrado e requer validação financeira operacional.',
        {
          code: 'FINANCIAL_ATTENTION_REQUIRED',
          attentionReason: currentTransition.financial_attention_reason,
          transitionId: currentTransition.id,
        }
      );
    }

    const transitionDto = mapTransitionToScheduledCancelResponseDto(currentTransition);

    return {
      ...(updatedBillingSub || ({} as BillingSubscriptionRecord)),
      cancel_at_period_end: true,
  ...
  async reconcilePaidToPaidEarlyActivationAdjustment(
    transitionId: string,
    workerId?: string,
    options?: { nowCommercialDate?: string; now?: Date }
  ): Promise<{ success: boolean; reason?: string; transition?: BillingTransitionV1Record }> {
    ...
    try {
      ...
      // 7. Avaliação de expiração do checkout attempt (Phase 3C.5B)
      const isAttemptExpired = currentAttempt
        ? isEarlyActivationCheckoutAttemptExpired(currentAttempt, claimed, now)
        : false;

      if (payments.length === 0) {
        // Crash B recovery: se a intenção de cancelamento anterior estava em 'attempting'
        if (currentAttempt && currentAttempt.cancel_state === 'attempting') {
          console.warn(
            `[CHECKOUT CLEANUP] Tentativa ${currentAttempt.attempt_id} encontrada em 'attempting' (Crash B). Marcando 'uncertain' sem blind retry.`
          );
          await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
            transitionId: claimed.id,
            ministryId: claimed.ministry_id,
            attemptId: currentAttempt.attempt_id,
            reason: 'crash_recovery_uncertain_cancel_attempt',
            nowIso: now.toISOString(),
          });
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return { success: false, reason: 'cancellation_outcome_uncertain' };
        }

        // Se o cancelamento estiver em 'uncertain': quarentena mantida, zero pagamentos não prova cancelamento, zero blind retry
        if (currentAttempt && currentAttempt.cancel_state === 'uncertain') {
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return { success: false, reason: 'cancellation_outcome_uncertain' };
        }

        // Se expirou localmente, NÃO chama cancelCheckout! Aguarda webhook oficial CHECKOUT_EXPIRED
        if (isAttemptExpired) {
          await this.billingRepo.releasePlanChangeLock(claimed.id);
          return {
            success: true,
            reason: 'local_expiry_awaiting_provider_webhook',
            transition: claimed,
          };
        }
      ...
      // Pending (Seção 12 / 3C.5B):
      if (singlePayment.status === 'PENDING') {
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        const reason = isAttemptExpired
          ? 'materialized_payment_blocks_checkout_cleanup'
          : 'payment_pending';
        return { success: true, reason, transition: claimed };
      }

      // Overdue (Seção 13 / 3C.5B):
  ...
  /**
   * Executa o protocolo atômico e seguro de cancelamento de um checkout avulso de early activation expirado (Phase 3C.5B).
   * Exige:
   * 1. Pre-Cancel Payment Gate já avaliado com zero pagamentos.
   * 2. Intenção durável de mutação com CAS antes do POST cancel.
   * 3. Chamada ao provedor com tratamento rigoroso de incertezas (timeout, 5xx -> uncertain quarantine; 401/403 -> operational; 404 -> not assumed safe).
   * 4. Post-Cancel Payment Gate: releitura obrigatória de cobranças após sucesso do POST.
   * 5. Finalização segura apenas se post-cancel retornar zero pagamentos.
   */
  async executeExpiredEarlyActivationCheckoutCleanup(params: {
    claimed: BillingTransitionV1Record;
    currentAttempt: BillingCheckoutAttempt;
    knownCheckoutId: string;
    now: Date;
    nowCommercialDate?: string;
  }): Promise<{ success: boolean; reason: string; transition?: BillingTransitionV1Record }> {
    const { claimed, currentAttempt, knownCheckoutId, now, nowCommercialDate } = params;
    const nowIso = now.toISOString();

    // Exigência canônica: cancelamento explícito via provedor só é permitido ANTES da expiração natural
    if (isEarlyActivationCheckoutAttemptExpired(currentAttempt, claimed, now)) {
      console.warn(
        `[CHECKOUT CLEANUP] Checkout ${knownCheckoutId} já atingiu expiração natural. Cancelamento explícito via provedor proibido.`
      );
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'checkout_already_expired_explicit_cancel_forbidden' };
    }

    // 0. Crash B recovery: se o cancel intent já estiver em 'attempting' sem resolução local
    ...
    // Se já estiver em 'uncertain': quarentena mantida, zero blind retry
    if (currentAttempt.cancel_state === 'uncertain') {
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'cancellation_outcome_uncertain' };
    }

    // Se já estiver cancelada com segurança: idempotência terminal
    if (currentAttempt.cancel_state === 'confirmed' || currentAttempt.status === 'canceled') {
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: true, reason: 'already_canceled', transition: claimed };
    }

    // 0.1 Preflight Payment Gate: se houver pagamentos materializados, aborta cancelamento (Seção 20)
    if (typeof this.provider.listPaymentsByCheckoutSession === 'function') {
      let prePayments: ProviderPaymentRecord[] = [];
      try {
        prePayments = await this.provider.listPaymentsByCheckoutSession(knownCheckoutId);
      } catch (preErr: any) {
        console.error(`[CHECKOUT CLEANUP] Falha ao listar pagamentos no preflight de ${knownCheckoutId}:`, preErr.message);
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: false, reason: 'pre_cancel_read_failed' };
      }

      const matchingPre = (prePayments || []).filter((p) => {
        if (p.customerId && claimed.provider_customer_id && p.customerId !== claimed.provider_customer_id) {
          return false;
        }
        return true;
      ...
      if (matchingPre.length > 0) {
        console.warn(`[CHECKOUT CLEANUP] Preflight encontrou pagamentos para ${knownCheckoutId}. Cancelamento abortado.`);
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: false, reason: 'pre_cancel_gate_found_payments' };
      }
    }

    // 1. Durabilidade da Intenção de Mutação (Seção 11): grava 'attempting' no Firestore via CAS antes do POST
    const cancellationIntentId = `cancel_ea_${claimed.id}_${currentAttempt.attempt_id}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
    try {
      await this.billingRepo.recordEarlyActivationCheckoutCancelAttempting({
        transitionId: claimed.id,
        ministryId: claimed.ministry_id,
        attemptId: currentAttempt.attempt_id,
      ...
      console.warn(`[CHECKOUT CLEANUP] Falha no CAS de cancel attempting para ${claimed.id}:`, casErr.message);
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'cancel_intent_cas_conflict' };
    }

    // 2. Chamada ao Provedor (POST /v3/checkouts/{id}/cancel) — Seções 1, 14, 15, 16, 17
    if (typeof this.provider.cancelCheckout !== 'function') {
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'provider_cancel_not_supported' };
    }

    try {
      await this.provider.cancelCheckout(knownCheckoutId);
    } catch (cancelErr: any) {
      const statusCode = (cancelErr as any)?.statusCode || (cancelErr as any)?.status;
      const msg = ((cancelErr as any)?.message || '').toLowerCase();
      const isAuth401 = statusCode === 401 || msg.includes('401') || msg.includes('unauthorized');
      const isForbidden403 = statusCode === 403 || msg.includes('403') || msg.includes('forbidden');
      const isNotFound404 = statusCode === 404 || msg.includes('404') || msg.includes('not found');

      if (isAuth401) {
        console.error(`[CHECKOUT CLEANUP] Falha operacional de autenticação (401) ao cancelar ${knownCheckoutId}:`, cancelErr.message);
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: false, reason: 'cancel_auth_failure_401' };
      }

      if (isForbidden403) {
        console.error(`[CHECKOUT CLEANUP] Falha operacional de permissão (403) ao cancelar ${knownCheckoutId}:`, cancelErr.message);
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: false, reason: 'cancel_auth_failure_403' };
      }

      if (isNotFound404) {
        // Seção 17: 404 não prova cancelamento seguro sem GET. Fail closed / verification pending.
        console.warn(`[CHECKOUT CLEANUP] Checkout ${knownCheckoutId} retornou 404 no cancel. Fail closed.`);
        await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
          transitionId: claimed.id,
          ministryId: claimed.ministry_id,
          attemptId: currentAttempt.attempt_id,
          reason: 'provider_cancel_404_not_found',
          nowIso,
        });
        await this.billingRepo.releasePlanChangeLock(claimed.id);
        return { success: false, reason: 'provider_cancel_404_not_found' };
      }

      // Timeout, 5xx, rede (Seção 15): OUTCOME UNCERTAIN
      console.error(`[CHECKOUT CLEANUP] Falha incerta ao cancelar checkout ${knownCheckoutId}:`, cancelErr.message);
      await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
        transitionId: claimed.id,
        ministryId: claimed.ministry_id,
        attemptId: currentAttempt.attempt_id,
    ...
    // 3. POST-CANCEL PAYMENT GATE (Seções 18, 19, 20, 21, 22, 44)
    if (typeof this.provider.listPaymentsByCheckoutSession !== 'function') {
      await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
        transitionId: claimed.id,
        ministryId: claimed.ministry_id,
        attemptId: currentAttempt.attempt_id,
        reason: 'provider_unsupported_list_payments_by_checkout',
        nowIso,
      });
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'provider_unsupported_list_payments_by_checkout' };
    }

    let postPayments: ProviderPaymentRecord[];
    try {
      postPayments = await this.provider.listPaymentsByCheckoutSession(knownCheckoutId);
    } catch (postErr: any) {
      // Seção 44: falha na leitura pós-cancel NÃO comuta para safe canceled.
      console.error(`[CHECKOUT CLEANUP] Falha no post-cancel payment read de ${knownCheckoutId}:`, postErr.message);
      await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
        transitionId: claimed.id,
        ministryId: claimed.ministry_id,
        attemptId: currentAttempt.attempt_id,
        reason: 'post_cancel_read_failed_verification_pending',
        nowIso,
      });
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'post_cancel_read_failed_verification_pending' };
    }

    if (!Array.isArray(postPayments)) {
      await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
        transitionId: claimed.id,
        ministryId: claimed.ministry_id,
        attemptId: currentAttempt.attempt_id,
        reason: 'malformed_provider_response_after_cancel',
        nowIso,
      });
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: false, reason: 'malformed_provider_response' };
    ...
    // Filtrar pagamentos compatíveis no pós-cancel
    const postMatching = postPayments.filter((p) => {
      if (p.customerId && claimed.provider_customer_id && p.customerId !== claimed.provider_customer_id) {
        return false;
      }
      return true;
    ...
    // 4. Se ZERO pagamentos comprovados no postflight (Seção 19): FINALIZAÇÃO SEGURA!
    if (postMatching.length === 0) {
      const updated = await this.billingRepo.recordEarlyActivationCheckoutCancelConfirmed({
        transitionId: claimed.id,
        ministryId: claimed.ministry_id,
        attemptId: currentAttempt.attempt_id,
        nowIso,
      ...
      await this.billingRepo.releasePlanChangeLock(claimed.id);
      return { success: true, reason: 'safe_canceled', transition: updated };
    }

    // 5. CORRIDA: Um ou mais pagamentos materializaram durante o cancel! (Seções 20, 21, 22)
    console.warn(`[CHECKOUT CLEANUP RACE] ${postMatching.length} pagamento(s) materializado(s) durante o cancelamento de ${knownCheckoutId}!`);

    // A) Múltiplos pagamentos no postflight
    if (postMatching.length > 1) {
      const settledMatching = postMatching.filter((p) => p.status === 'CONFIRMED' || p.status === 'RECEIVED');
      for (const sp of settledMatching) {
        const spTxId = `${this.provider.name}_${sp.id}`;
        const spPaidDate = (sp as any).confirmedDate || sp.clientPaymentDate || sp.paymentDate || getBillingDate(now);
        await this.billingRepo.saveTransaction({
          id: spTxId,
          ministry_id: claimed.ministry_id,
          provider: this.provider.name,
          provider_payment_id: sp.id,
          amount_cents: sp.amountCents ?? claimed.current_early_activation_quote?.prorated_adjustment_cents ?? 0,
          currency: 'BRL',
          status: 'paid',
          due_date: sp.dueDate || spPaidDate,
          paid_at: nowIso,
      ...
      return {
        success: false,
        reason: 'EARLY_ADJUSTMENT_MULTIPLE_PROVIDER_PAYMENTS',
        transition: reloaded && isBillingTransitionV1(reloaded) ? reloaded : undefined,
      };
    }

    // B) Exatamente um pagamento materializado
    const singlePostPayment = postMatching[0];

    // B.1) Se liquidado (CONFIRMED ou RECEIVED) — Seção 21
    if (singlePostPayment.status === 'CONFIRMED' || singlePostPayment.status === 'RECEIVED') {
      const synthEvent: ParsedWebhookEvent = {
        ...
        status: singlePostPayment.status as any,
        confirmedDate: (singlePostPayment as any).confirmedDate || singlePostPayment.clientPaymentDate || singlePostPayment.paymentDate,
        paymentDate: singlePostPayment.paymentDate,
        paymentMethod: singlePostPayment.billingType as any,
        invoiceUrl: singlePostPayment.invoiceUrl,
      };
      ...
      const reloaded = await this.billingRepo.getTransitionById(claimed.id, claimed.ministry_id);
      return {
        success: settlementResult.processed && settlementResult.reason === 'early_activation_settled_and_promoted',
        reason: settlementResult.reason || 'early_activation_settled_race',
        transition: reloaded && isBillingTransitionV1(reloaded) ? reloaded : undefined,
      };
    }

    // B.2) Se PENDING ou OVERDUE — Seção 22
    if (singlePostPayment.status === 'PENDING' || singlePostPayment.status === 'OVERDUE') {
      console.warn(`[CHECKOUT CLEANUP RACE] Pagamento ${singlePostPayment.status} materializado no postflight. Não declara safe cancel.`);
      await this.billingRepo.updateTransition(claimed.id, claimed.ministry_id, {
        financial_attention_required: true,
      ...
      return {
        success: false,
        reason: 'materialized_payment_blocks_checkout_cleanup',
        transition: reloaded && isBillingTransitionV1(reloaded) ? reloaded : undefined,
      };
    }

    // Outro status: fail closed
    await this.billingRepo.releasePlanChangeLock(claimed.id);
    return { success: false, reason: `unexpected_post_cancel_payment_status_${singlePostPayment.status}` };
  }
  ...
  async processEarlyActivationAdjustmentSettlement(
    ...
    // Canceled/Expired attempt race (Seção 29, 30, 31)
    if (matchedAttempt.status === 'canceled' || matchedAttempt.status === 'expired') {
      if (parsedEvent?.eventType === 'payment_confirmed' || parsedEvent?.eventType === 'payment_received') {
        console.error(
          `[CANCELED ATTEMPT PAYMENT CONFLICT] Pagamento liquidado em tentativa cancelada/expirada ${matchedAttempt.attempt_id}. Gravando na ledger como stale fact sem conceder entitlement.`
        );
        return await this.recordStaleSettledPaymentLedger({
          planChange,
          matchedAttempt,
          ...
          nowIso,
          ministryId,
          attentionReason: 'CANCELED_ATTEMPT_WITH_SETTLED_PAYMENT',
        });
      }
    ...
    if (parsedEvent?.eventType === 'checkout_expired') {
      ...
      // CHECKOUT_EXPIRED Payment Safety Gate (Seção 6, 7, 8, 9, 10, 11)
      const chkId = matchedAttempt.provider_checkout_id || parsedEvent.providerCheckoutId;
      if (chkId && typeof this.provider.listPaymentsByCheckoutSession === 'function') {
        let sessionPayments: ProviderPaymentRecord[] = [];
        try {
          sessionPayments = await this.provider.listPaymentsByCheckoutSession(chkId);
        } catch (postErr: any) {
          console.error(`[WEBHOOK CHECKOUT_EXPIRED] Falha ao listar pagamentos da sessão ${chkId}:`, postErr.message);
          await this.billingRepo.markEarlyActivationCheckoutExpiredPending({
            transitionId: planChange.id,
            ministryId,
        ...
        const matchingPayments = Array.isArray(sessionPayments)
          ? sessionPayments.filter((p) => {
              if (p.customerId && planChange.provider_customer_id && p.customerId !== planChange.provider_customer_id) {
                return false;
              }
              return true;
            })
        ...
        // Pagamento liquidado observado no gate pós-expiração
        const settled = matchingPayments.filter((p) => p.status === 'CONFIRMED' || p.status === 'RECEIVED');
        if (settled.length === 1) {
          const sp = settled[0];
          const synthEvent: ParsedWebhookEvent = {
            ...
            confirmedDate: (sp as any).confirmedDate || sp.clientPaymentDate || sp.paymentDate,
            paymentDate: sp.paymentDate,
            paymentMethod: sp.billingType as any,
            invoiceUrl: sp.invoiceUrl,
          };
          const settlementResult = await this.processEarlyActivationAdjustmentSettlement(
            synthEvent,
            planChange,
        ...
        if (matchingPayments.some((p) => p.status === 'PENDING' || p.status === 'OVERDUE')) {
          await this.billingRepo.updateTransition(planChange.id, ministryId, {
            financial_attention_required: true,
            financial_attention_reason: 'materialized_payment_blocks_checkout_cleanup',
            financial_safety_status: 'attention_required',
        ...
        // Zero pagamentos comprovados: finalização segura como EXPIRED (Seção 7)
        await this.billingRepo.recordEarlyActivationCheckoutExpiredConfirmed({
          transitionId: planChange.id,
          ministryId,
          attemptId: matchedAttempt.attempt_id,
          nowIso,
      ...
      // Fallback se provider não possui listPaymentsByCheckoutSession
      matchedAttempt.status = 'expired';
      matchedAttempt.provider_session_terminal = true;
      matchedAttempt.failure_classification = 'session_expired';
      matchedAttempt.expiry_confirmed_at = nowIso;
      matchedAttempt.completed_at = nowIso;
      await this.billingRepo.updateTransition(planChange.id, ministryId, {
        checkout_attempts: planChange.checkout_attempts,
      });
    ...
    if (parsedEvent?.eventType === 'checkout_canceled') {
      ...
      // CHECKOUT_CANCELED payment safety gate antes de confirmar cancelamento seguro (Seção 17)
      const chkId = matchedAttempt.provider_checkout_id || parsedEvent.providerCheckoutId;
      if (chkId && typeof this.provider.listPaymentsByCheckoutSession === 'function') {
        let sessionPayments: ProviderPaymentRecord[] = [];
        try {
          ...
          console.error(`[WEBHOOK CHECKOUT_CANCELED] Falha ao listar pagamentos da sessão ${chkId}:`, postErr.message);
          await this.billingRepo.markEarlyActivationCheckoutCancelUncertain({
            transitionId: planChange.id,
            ministryId,
            attemptId: matchedAttempt.attempt_id,
        ...
        // Zero pagamentos comprovados: finalização segura como CANCELED (Seção 17)
        await this.billingRepo.recordEarlyActivationCheckoutCancelConfirmed({
          transitionId: planChange.id,
          ministryId,
          attemptId: matchedAttempt.attempt_id,
          nowIso,
      ...
      // Fallback
      matchedAttempt.status = 'canceled';
      matchedAttempt.cancel_state = 'confirmed';
      matchedAttempt.provider_session_terminal = true;
      matchedAttempt.failure_classification = 'session_canceled';
      matchedAttempt.cancellation_confirmed_at = nowIso;
      matchedAttempt.completed_at = nowIso;
      await this.billingRepo.updateTransition(planChange.id, ministryId, {
        checkout_attempts: planChange.checkout_attempts,
      });
  ...
  private async recordStaleSettledPaymentLedger(params: {
    ...
    const { planChange, matchedAttempt, parsedEvent, now, nowIso, ministryId, attentionReason } = params;
    const exactPaymentId = parsedEvent?.providerPaymentId || matchedAttempt.provider_payment_id;

    const defaultReason = attentionReason || 'STALE_ATTEMPT_EARLY_ADJUSTMENT_SETTLED';

    if (!exactPaymentId) {
      return { status: 'ok', processed: false, reason: defaultReason };
    }
    ...
    const reason = txConflict ? 'FINANCIAL_TRANSACTION_CONFLICT' : defaultReason;
  ...
  async createEarlyActivationQuote(
    ...
    const nowDate = options?.now
      ? (typeof options.now === 'string' ? new Date(options.now) : options.now)
      : new Date();

    if (isEarlyAdjustmentObligationFinanciallyLive(transition, nowDate)) {
      const state = classifyEarlyAdjustmentFinancialState(transition, nowDate);
      throw new AppError(
        409,
