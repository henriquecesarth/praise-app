...
- Billing Transition V1 Status:
  ...
  - Phase 3B.3A (Scheduled Renewal Settlement, Immutable Snapshot Activation & Crash-Safety Hardening): COMPLETE & AUTOMATED TEST SUITE PASSING (Two-Gate Model cumulativo: Gate A Financeiro [pagamento CONFIRMED ou RECEIVED] + Gate B Comercial [currentCommercialDate >= effective_billing_date]; PLAN ID != ENTITLEMENT SNAPSHOT: snapshot de cotas comprado em requested_at é imutável e aplicado via applyLockedEntitlementSnapshot, tornando a ativação imune a catalog drift posterior em plans.config.ts; autoridade financeira exata vinculada a payment.id === future_provider_payment_id e payment.dueDate === effective_billing_date [desvio de vencimento gera TARGET_FIRST_PAYMENT_BOUNDARY_MISMATCH com slot HELD]; pagamentos de ciclo posterior roteados como SECOND_CYCLE_PAYMENT sem afetar a transição original; Early settlement pré-fronteira persiste canonical BillingTransaction e provas de liquidação write-once, mantendo transição scheduled, entitlement runtime na source e slot HELD; revalidação fresh de contrato alvo [ACTIVE, customer, cycle, recurring value]; estorno/chargeback pré-fronteira bloqueia ativação; Activation Local Completion Gate valida convergência completa [appSub, billingSub, transaction, payment identity] antes de confirmar completed; Matriz de crash-recovery A-F prova retenção estrita de slot HELD em todos os estados parciais e liberação exclusivamente pós-convergência; reconciliador assíncrono BillingReconcilerWorker recupera transições scheduled de falhas parciais de forma idempotente; fronteira com pagamento pendente mantém scheduled, cotas de origem ativas e slot HELD; Phase 3B.3B Renewal Failure / Grace: NOT STARTED; Phase 3C Early Activation: NOT STARTED).
