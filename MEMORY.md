...
- Billing Transition V1 Status:
  ...
  - Phase 3C (Early Activation):
    ...
    - Phase 3C.5A (Early Activation Known-Checkout Reconciliation Worker, Fairness & Legacy Normalization Hardening): COMPLETE & TEST MATRIX VALIDATED (Reconciliador assíncrono `reconcilePaidToPaidEarlyActivationAdjustment` integrado ao `BillingReconcilerWorker`; opera estritamente quando `provider_checkout_id` já é conhecido via `listPaymentsByCheckoutSession(checkoutId)` correspondente a `GET /v3/payments?checkoutSession=<checkoutId>`; submete deterministicamente os pagamentos observados à state machine canônica da Phase 3C.4 `processEarlyActivationAdjustmentSettlement`; MULTIPLE PROVIDER PAYMENT RECORDS FOR ONE DETACHED OBLIGATION ARE A FINANCIAL SAFETY CONFLICT; SETTLED MEMBERS OF AN AMBIGUOUS SET ARE STILL LEDGER FACTS: qualquer cobrança liquidada em conjunto ambíguo entra no ledger canônico como `BillingTransaction` idempotente com ID `${provider}_${provider_payment_id}`, acionando `EARLY_ADJUSTMENT_MULTIPLE_PROVIDER_PAYMENTS` sem concessão de entitlement, slot HELD e zero refund; proteção contra `FINANCIAL_TRANSACTION_CONFLICT` em colisão de amount/date; checagem estrita de Slot Ownership antes de conceder direito de uso com fail closed; quarentena estrita mantida para attempts incertos sem checkout ID conhecido [zero blind retry, zero discovery inventado]; tratamento de zero payments como benigno com retenção de slot HELD; reconciliação de tentativas stale com registro canônico no ledger contábil sem entitlement; Commercial Boundary Guard acionando atenção financeira sem auto-estorno; tratamento de pagamentos pendentes, vencidos, estornados e contestados; classificação rigorosa de falhas do provedor: transitórias [timeouts/network/5xx -> `provider_read_failure`] vs operacionais de autenticação/permissão [401 -> `provider_auth_failure_401`, 403 -> `provider_auth_failure_403`] vs respostas malformadas [`malformed_provider_response` fail closed]; RECONCILER FAIRNESS, PAGINATION & STARVATION HARDENING: LIMIT IS NOT PAGINATION: bounding puro sem cursor de progresso causa starvation permanente; introduzido campo operacional `last_reconciled_at` totalmente separado de datas financeiras/comerciais; BOUNDED NORMALIZATION PASS WITH DURABLE CURSOR: o worker executa passo bounded idempotente no início do ciclo via `normalizeLegacyTransitionsWithoutScheduling(50)`; varredura ordenada por Document ID (`__name__ ASC`) com cursor durável persistido em `billing_schedulers/normalization_{provider}` que sobrevive a crashes e restarts do worker; elimina starvation no próprio discovery da normalização [documentos normalizados não bloqueiam a varredura dos lotes subsequentes]; wrap controlado ao final da faixa sem hot loop; at-least-once safety [normaliza registros primeiro, persiste avanço do cursor depois]; fresh transaction check atômico que JAMAIS sobrescreve timestamp real de agendamento; zero mutação em `updated_at` e zero mutação em campos de negócio; novas transições nascem com `last_reconciled_at: null` por padrão; UNIFIED MODERN LRR QUERY: query canônica única indexada por `last_reconciled_at ASC` e `__name__ ASC`, onde `null` é indexado como o menor valor e assume topo absoluto de scheduling FIFO [never reconciled = prioridade máxima]; sem dependência de `created_at` na query do banco prevenindo exclusão acidental de registros históricos sem data de criação; Round-Robin Interleaving multi-bucket; autoridade única de scheduling: `last_reconciled_at` gravado atomicamente no claim e NÃO alterado na liberação do lock; isolamento estrito de buckets operacionais (`filterHealthy`) para que atenção financeira não bloqueie transições saudáveis; sobrevivência determinística a restarts de worker e CAS lock multi-worker seguro; exatamente 2 composite indexes versionados em `backend/firestore.indexes.json` e validados por teste estático; 78 testes dedicados cobrindo integralmente as matrizes da 3C.5A e os testes de normalização com cursor, 100/125 legacy, mixed, restart, crashes e index contracts; 831 testes no backend e 48 testes no web passando).
