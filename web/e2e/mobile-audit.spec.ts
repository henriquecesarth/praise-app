...
async function expectNoHorizontalOverflow(page: Page) {
  ...
  expect(dimensions.scrollWidth, `document overflowed: ${JSON.stringify(dimensions)}`).toBeLessThanOrEqual(dimensions.clientWidth);

  const clippedControls = await page.locator('button, a, input, select, textarea, [role="tab"]').evaluateAll((elements) => {
    ...
    return elements.flatMap((element) => {
      ...
      if (!isRendered || (rect.left >= -1 && rect.right <= viewportWidth + 1)) return [];

      return [{
        name: htmlElement.getAttribute('aria-label') || htmlElement.textContent?.trim().replace(/\s+/g, ' ').slice(0, 80) || htmlElement.tagName,
  ...
  expect(clippedControls, `interactive controls were clipped: ${JSON.stringify(clippedControls)}`).toEqual([]);
}
