...
| Tenancy | Ministry é a fronteira principal usada pela UI e pela maioria das rotas. Papéis básicos são admin/member. | ministries feature, rbac.ts |
...
| INC-002 | Ministry authorization | getMinistryById retorna role member quando o usuário não é owner e nenhuma associação é encontrada. requireMinistryRole usa esse resultado; conhecer um ministryId pode ser tratado como membership. | MinistryRepository.getMinistryById, middleware/rbac.ts |
| INC-003 | Subscription guard | requireActiveSubscription ignora erros diferentes de AppError 402 e chama next. Falhas de leitura podem liberar a operação. | middleware/rbac.ts |
...
INC-011 foi removido desta lista após a UI passar o ministério ativo e cancelar solicitações obsoletas, com teste automatizado. Os demais itens continuam documentados e fora do escopo desta correção.

## Incomplete Implementation

| ID | Area | Current state | Evidence |
| --- | --- | --- | --- |
| GAP-001 | Backend tests | O backend não possui suíte ou script de testes. O web possui Vitest/Testing Library e Playwright. | backend/package.json, web/package.json, web/src/**/*.test.*, web/e2e/ |
...
- Compatibilidade com emuladores Firebase; nenhuma configuração de emulator foi encontrada.
- Funcionamento E2E com credenciais reais; não foi realizado para evitar uso de serviços externos/dados.
- Estado de assinatura e integração de pagamentos; o código guarda subscription_status, mas não integra um provedor de pagamento.
