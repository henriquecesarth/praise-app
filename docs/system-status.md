...
| WhatsApp Onboarding & Provider Credential Acquisition | Backend de onboarding WhatsApp da Meta Cloud API (Phase 7D1/7D1-R1/7D1-R2; contratos de liveness e recuperação Phase 7D1-A-R3, Phase 7D1-A-R4 e Phase 7D1-A-R5 formalizados eliminando retenção de capacidade pré-materialização e reconciliando máquina de estados de sessões efêmeras, prazo rígido de 24h não-deslizante sem extensão por rotação de sessão, ponteiro de sessão única no connection com compare-current e guarda anti-sessão obsoleta, alvo de retomada determinístico via `resumeConnectionId`, compensação segura de provedor em expiração e serialização estrita de concorrência). Alocação de conexões pendentes (`pending`) com prazo rígido de 24 horas (`pending_expires_at = createdAt + 24h`) e sessões efêmeras (`whatsapp_onboarding_sessions`) de 15 minutos com verificação CSRF em tempo constante via hash SHA-256 (`state_nonce_hash`), reserva atômica de capacidade com contenção OCC em `organizations`, gating estrito exigindo `billingAccessMode === 'normal'` para novas reservas e permissão de conclusão/retomada estagiada durante grace (DEC-7D-16), transição atômica de reservas pendentes expiradas para `disconnected` (`PENDING_EXPIRED`) liberando genuinamente a cota configurada com compensação condicional de webhooks para WABA compartilhada (DEC-7D-19), liberação atômica imediata de reservas pendentes não materializadas (`disconnected`) em falhas pré-materialização terminais (CSRF inválido, suspensão prévia, falha na troca de código OAuth, WABA/telefone não autorizados, claim colidida) evitando bloqueio de 24 horas, saga de 13 passos para aquisição e estágio progressivo de credenciais criptografadas (`token_type: 'business_token'`) com atomicidade de estágio (segredo + sessão), verificação de autoridade no Messaging Account Container (`verifyMessagingAccountAccess`) e edge de telefones (`listAuthorizedPhoneNumbers`), normalização estrita E.164, registro efêmero de PIN em memória (zero persistência/logging), assinatura de webhooks (`subscribeMessagingAccountApps`), rechecagem transacional autoritativa de capacidade comercial e modo de acesso no Passo 10 abortando antes de qualquer aquisição de claim em caso de downgrade/estouro de cota com expurgo imediato de segredo estagiado (ou retenção não-destrutiva para regularização de assinatura com retomada via `POST /start` com `resumeConnectionId` em `mode = 'resume_staged'`), guarda contra sessões obsoletas (`ONBOARDING_SESSION_SUPERSEDED`), idempotência em replay de conclusão, materialização atômica em transação com claim determinístico `claim_meta_${phoneNumberId}`, prevenção de credenciais órfãs e retenção de segredo para retry seguro em falhas transitórias, rotas REST `POST /organizations/:orgId/whatsapp/onboarding/start` e `POST /organizations/:orgId/whatsapp/onboarding/complete`. (META_LIVE_E2E_VALIDATION: NOT_EXECUTED; validado via suíte de testes Vitest com mocks de rede). | backend/src/features/whatsapp/meta-whatsapp.provider.ts, backend/src/features/whatsapp/whatsapp-connection.service.ts, backend/src/repositories/WhatsAppOnboardingSessionRepository.ts, organization.routes.ts, meta-whatsapp.provider.test.ts, whatsapp.onboarding.test.ts |
