   ...
   - `resolveAccessMode` converte a assinatura para `restricted_over_limit`.
      ...
      - **PHASE 3C EARLY ACTIVATION**: **IMPLEMENTED, HARDENED & SANDBOX HOMOLOGATED**.
        ...
        - **Phase 3C.6 (Early Activation End-to-End Sandbox Homologation)**: **COMPLETE & SANDBOX HOMOLOGATED**.
          - *Scenario A3 (Authoritative Canonical Entitlement E2E Proof)*: Homologação canônica ponta a ponta sem intervenção manual no Asaas Sandbox. Transição criada exclusivamente via helpers de domínio de produção (`buildTransitionCommercialSnapshot`, `buildBillingTransitionV1Record`, `createTransitionAndClaimSlot`). Provou as cotas reais do catálogo `PLANS_CATALOG`: Lite (20 membros / 100 músicas) -> Essential (40 membros / 200 músicas). Cotação pró-rata sob relógio real em 1267 centavos (R$ 12,67 sobre 19/30 dias). Checkout avulso único com cardinalidade 1. Liquidação operada por `PAYMENT_CONFIRMED`. Exatamente 1 `BillingTransaction` canônica no ledger contábil. Cotas bloqueadas persistidas em `ministry_subscriptions` (`locked_member_quota = 40`, `locked_song_quota = 200`) e validadas em `SubscriptionService.getSubscriptionSummary` (Essential 40/200). Preservação estrita do ciclo de origem (`current_period_end` inalterado), transição mantida `scheduled`, slot global retido em `HELD`, recorrências intactas e reconciliador 100% idempotente (NO-OP).
          - *Scenario A2 Quota Fixture Contamination Disclosure*: O Scenario A2 homologou com sucesso o fluxo financeiro de checkout, pagamento no Sandbox, liquidação, ledger e idempotência sem nenhum reparo manual no Firestore. Porém, a fixture de teste montou manualmente os snapshots copiando valores arbitrários de testes unitários (`5/50 -> 15/200`). O Scenario A2 NÃO é a prova canônica das quotas do catálogo de produtos. Causa-raiz: reutilização acidental de mocks de teste unitário no script de teste; regra de homologação: *SANDBOX HOMOLOGATION FIXTURES MUST NOT COPY UNIT-TEST SNAPSHOTS*.
          - *Scenario A1 Slot Contamination Disclosure*: Retido como contaminado por formato incorreto de slot ID no script de teste (separador simples `_`), corrigido no A2 e A3 pela função canônica `buildActiveTransitionSlotId(ministryId, provider)` (`__`).
          - *Snapshot Authority Chain*: `PLANS_CATALOG (Lite 20/100 -> Essential 40/200)` -> `buildTransitionCommercialSnapshot` -> `target_entitlement_snapshot (40/200)` -> `PAYMENT_CONFIRMED` -> `processEarlyActivationAdjustmentSettlement` -> `applyLockedEntitlementSnapshot` -> `ministry_subscriptions (locked_member_quota = 40, locked_song_quota = 200)` -> `SubscriptionService.getSubscriptionSummary` -> `Essential (40/200)`.
