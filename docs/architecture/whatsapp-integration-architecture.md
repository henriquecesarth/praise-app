...
- **`EXECUTOR_CLASS: EXTERNAL_BEST_EFFORT_SCHEDULER`**: Best-effort external HTTP scheduler trigger. Durability and eventual consistency are enforced by Cloud Firestore leases, transactional reservations, and retry schedules.
...
- **`EXECUTION_DEADLINE_PROPAGATION (Phase 7D1-B-R3-R1)`**: Implementation-enforced execution deadline (`WhatsAppExecutionDeadline`) designed to return within the 30-second scheduler envelope (`<= 24-25s`). Operates with a 24-second execution budget (`budgetMs: 24_000`) and 1.5s safety margin (`safetyMarginMs: 1_500`), dynamically clamping downstream provider timeouts (`getClampedTimeoutMs`), enforcing budget checks before attempt reservation (<3000ms) and verification (<2000ms), and bounding pagination safely (<1000ms) without lost leases, dropped attempts, or unhandled crashes.
